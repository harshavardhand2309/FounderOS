...
// One source of truth for the hero trailer, so the poster, the <video> and the
// hover-warm can never drift apart. The poster used to be home-intro-poster.jpg
// — the *background montage's* first frame, a different clip — so opening the
// trailer flashed an unrelated still before the real footage arrived.
const TRAILER = '/assets/Finaldraft03.mp4'
const TRAILER_POSTER = '/assets/Finaldraft03-poster.jpg'
      ...
      <header ref={heroRef} onMouseEnter={() => setHeroHover(true)} onMouseLeave={() => setHeroHover(false)} style={css('position:relative;z-index:10;min-height:100vh;overflow:hidden;display:flex;flex-direction:column')}>
        ...
        <nav className="ag-rv ag-rv-top ag-pad" style={{ ...css('position:relative;z-index:2;display:flex;align-items:center;justify-content:space-between;padding:24px 48px'), transitionDelay: '.05s' }}>
          <Link to="/" style={css('display:flex;align-items:center;gap:11px')}>
            <img src="/assets/Logo.png" alt="" aria-hidden="true" style={css('width:34px;height:34px;border-radius:8px;object-fit:cover;display:block')} />
      ...
      <footer style={css('position:relative;z-index:10;background:#f6f6f4;padding:66px 48px 30px')}>
        <div style={css('max-width:1240px;margin:0 auto')}>
          ...
          <div style={css('display:flex;flex-direction:column;align-items:flex-start;gap:14px;margin-bottom:46px')}>
            <img src="/assets/Logo.png" alt="Lvl-Up Sports crest" style={css('width:64px;height:64px;border-radius:13px;object-fit:cover;display:block')} />
